...
def stt_uses_external_turns(user_config) -> bool:
    ...
    if user_config.stt.provider == ServiceProviders.DASHSCOPE.value:
        return True
    return False
...
class DograhQwenLLMService(QwenLLMService):
    """Qwen client tuned for voice use over cross-region TLS connections."""
    ...
    def create_client(
        ...
        **kwargs,
        ...
        # Voice turns must fail fast. Retrying a slow DashScope request inside
        # the SDK can otherwise hold the UI for more than thirty seconds; the
        # direct demo has a local FAQ-backed response guard for this case.
        return AsyncOpenAI(
            api_key=api_key,
            base_url=base_url,
    ...
    async def prewarm(self) -> None:
        """Warm the reusable HTTP/model path with a one-token generic request.

        Fast-opening voice calls overlap this tiny request with the fixed
        greeting. It contains no workflow prompt or visitor data and is ignored
        on failure; the real conversation remains authoritative.
        """
...
def create_stt_service(
    ...
    elif user_config.stt.provider == ServiceProviders.DASHSCOPE.value:
        websocket_url = user_config.stt.websocket_url
        _validate_runtime_service_url(websocket_url, "websocket_url")
        language = getattr(user_config.stt, "language", None)
        language_hints = (
            [language] if language and str(language).lower() != "auto" else None
        )
        configured_sample_rate = getattr(user_config.stt, "sample_rate", None)
        input_sample_rate = audio_config.transport_in_sample_rate
        if configured_sample_rate and configured_sample_rate != input_sample_rate:
            logger.warning(
                "DashScope STT configured sample rate {} differs from the "
                "transport's {} Hz; sending the actual transport rate",
                configured_sample_rate,
                input_sample_rate,
            )
        return DashScopeSTTService(
            api_key=user_config.stt.api_key,
            workspace_id=getattr(user_config.stt, "workspace_id", None),
            base_url=websocket_url,
            model=user_config.stt.model,
            sample_rate=input_sample_rate,
            format=getattr(user_config.stt, "audio_format", "pcm"),
            language_hints=language_hints,
            vocabulary_id=getattr(user_config.stt, "vocabulary_id", None),
...
def create_tts_service(
    ...
    if user_config.tts.provider == ServiceProviders.DEEPGRAM.value:
    ...
    elif user_config.tts.provider == ServiceProviders.MINISTREAM.value:
        # The MiniStream implementation is intentionally imported here: this
        # lets configuration ship independently from the optional runtime
        # service package.
        from api.services.ministream.tts import MiniStreamTTSService

        websocket_url = user_config.tts.websocket_url
        _validate_runtime_service_url(websocket_url, "websocket_url")
        return MiniStreamTTSService(
            api_key=user_config.tts.api_key,
            model=user_config.tts.model,
            websocket_url=websocket_url,
            language=user_config.tts.language,
            generation_mode=user_config.tts.generation_mode,
            voice_preset_key=user_config.tts.voice_preset_key,
            buffer=user_config.tts.buffer,
            buffer_idle_ms=user_config.tts.buffer_idle_ms,
            max_generate_length=user_config.tts.max_generate_length,
            sample_rate=user_config.tts.sample_rate,
            playback_rate=user_config.tts.playback_rate,
            verify_ssl=user_config.tts.verify_ssl,
            tls_certificate_fingerprint_sha256=(
                user_config.tts.tls_certificate_fingerprint_sha256
            ),
            text_filters=[xml_function_tag_filter],
            skip_aggregator_types=["recording_router", "recording"],
        )
    elif user_config.tts.provider == ServiceProviders.DASHSCOPE.value:
        websocket_url = user_config.tts.websocket_url
        _validate_runtime_service_url(websocket_url, "websocket_url")
        return DashScopeTTSService(
            ...
            sample_rate=getattr(user_config.tts, "sample_rate", 24000),
            audio_format=getattr(user_config.tts, "audio_format", "pcm"),
            instruction=getattr(user_config.tts, "instruction", None),
            rate=getattr(user_config.tts, "rate", 1.0),
            base_url=websocket_url,
            workspace_id=getattr(user_config.tts, "workspace_id", None),
            text_filters=[xml_function_tag_filter],
            skip_aggregator_types=["recording_router", "recording"],
            silence_time_s=1.0,
        )
...
def create_llm_service_from_provider(
    ...
    *,
    ...
    elif provider == ServiceProviders.DASHSCOPE.value:
        base_url = base_url or "https://dashscope.aliyuncs.com/compatible-mode/v1"
        _validate_runtime_service_url(base_url, "base_url")
        return DograhQwenLLMService(
            api_key=api_key,
            base_url=base_url,
...
def create_llm_service(
    ...
    if provider in (
        ...
        kwargs["enable_thinking"] = getattr(user_config.llm, "enable_thinking", False)
        kwargs["max_tokens"] = getattr(user_config.llm, "max_tokens", 256)
    elif provider == ServiceProviders.OPENROUTER.value:
        kwargs["base_url"] = user_config.llm.base_url
    elif provider == ServiceProviders.AZURE.value:
