...
class PipecatEngine:
    def __init__(
        ...
        *,
        ...
        self.task = task
        self.llm = llm
        self._client_opening_committed = False
        ...
        # Protect non-interruptible nodes from microphone noise while the LLM
        # is generating and before the first TTS audio frame starts playing.
        self._llm_response_in_progress: bool = False
        self._awaiting_llm_playback: bool = False
    ...
    async def queue_node_opening(
        ...
        *,
        ...
        if (
            ...
            and self._call_context_vars.get(CLIENT_OPENING_CONTEXT_KEY) is True
        ):
            transcript = self._call_context_vars.get(
                CLIENT_OPENING_TRANSCRIPT_CONTEXT_KEY
            )
            if (
                not self._client_opening_committed
                ...
                self._client_opening_committed = True
            logger.debug("Client-preloaded opening selected; skipping server greeting")
            return "none"
    ...
    async def should_mute_user(self, frame: "Frame") -> bool:
        ...
        if isinstance(frame, BotStartedSpeakingFrame):
            ...
            self._awaiting_llm_playback = True
            ...
            if not self._llm_response_in_progress:
                self._awaiting_llm_playback = False
            self._queued_speech_mute_state = "idle"
            self._speech_playback_finished.set()
        elif isinstance(frame, LLMFullResponseStartFrame):
            self._llm_response_in_progress = True
            self._awaiting_llm_playback = True
        elif isinstance(frame, LLMFullResponseEndFrame):
            self._llm_response_in_progress = False
            # Pure tool calls and empty/error responses produce no TTS audio.
            if not self._current_llm_generation_reference_text.strip():
                self._awaiting_llm_playback = False
        ...
        if (
            ...
            and (self._llm_response_in_progress or self._awaiting_llm_playback)
        ):
            return True
