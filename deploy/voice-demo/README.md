...
独立 Caddy 容器在 `80/443` 为公网 IP 自动申请 Let's Encrypt
`shortlived` HTTPS 证书。证书有效期较短，但 Caddy 会自动续期。
...
7. 确认 `https://<公网IP>` 可访问。若服务器未来启用共享反向代理，
   可停用 `edge` 服务，再把新域名转发到 `http://127.0.0.1:18080`。
