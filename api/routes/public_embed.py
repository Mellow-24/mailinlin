...
_CLIENT_OPENING_SETTING_KEYS = (
    "clientOpeningAssetId",
    "clientOpeningAudioUrl",
    "clientOpeningTranscript",
    "clientOpeningDurationMs",
)


def _safe_public_embed_settings(settings: dict | None) -> dict:
    """Copy settings while removing browser-unsafe preloaded opening assets."""
    safe_settings = dict(settings or {})
    audio_url = safe_settings.get("clientOpeningAudioUrl")
    if audio_url is not None and not is_browser_safe_local_public_audio_path(audio_url):
        for key in _CLIENT_OPENING_SETTING_KEYS:
...
class InitEmbedResponse(BaseModel):
    ...
    # A static text greeting from the exact published definition pinned to this
    # run. Voice widgets can display it immediately while media services warm
    # up; real-time TTSText events remain the source of playback timing.
    opening_transcript: str | None = None
...
def _static_opening_transcript(workflow) -> str | None:
    """Return a safe preview for a fixed text greeting on the published run.

    Dynamic templates, pre-call fetches, audio greetings, and per-call greeting
    overrides must wait for the live pipeline because their final text is not
    known at init time.
    """

    definition = getattr(workflow, "released_definition", None) or getattr(
    ...
    data = start_nodes[0].get("data")
    if not isinstance(data, dict):
        return None
    if (data.get("greeting_type") or "text") != "text":
        return None
    if (data.get("pre_call_fetch_mode") or "disabled") != "disabled":
        return None
    ...
    if not greeting or "{{" in greeting or "{%" in greeting:
        return None
    return greeting
...
async def initialize_embed_session(
    ...
    token_settings = _safe_public_embed_settings(embed_token.settings)
    widget_type = "chat" if token_settings.get("widgetType") == "chat" else "voice"
    ...
    try:
        ...
        if is_chat:
            ...
            fast_opening = bool(
                token_settings.get("fastOpening")
                and token_settings.get("embedMode") == "headless"
            )
            client_opening_audio_url = token_settings.get("clientOpeningAudioUrl")
            client_opening_transcript = token_settings.get("clientOpeningTranscript")
            client_opening = bool(
                fast_opening
                and is_browser_safe_local_public_audio_path(client_opening_audio_url)
                and isinstance(client_opening_transcript, str)
                and client_opening_transcript.strip()
            )
            direct_voice_demo = bool(
                fast_opening and token_settings.get("directVoiceDemo") is True
            )
            initial_context = {
                **context_variables,
                ...
                # Server-owned capability: visitor context cannot opt arbitrary
                # workflows into deferred STT startup.
                FAST_OPENING_CONTEXT_KEY: fast_opening,
                CLIENT_OPENING_CONTEXT_KEY: client_opening,
                CLIENT_OPENING_TRANSCRIPT_CONTEXT_KEY: (
                    client_opening_transcript.strip()[:1000] if client_opening else ""
                ),
    ...
    if not is_chat and "greeting_override" not in context_variables:
        client_opening_transcript = initial_context.get(
            CLIENT_OPENING_TRANSCRIPT_CONTEXT_KEY
        )
        opening_transcript = (
            client_opening_transcript
            if client_opening_transcript
            else _static_opening_transcript(workflow)
        )
...
async def get_embed_config(token: str, request: Request, response: Response):
    ...
    settings = _safe_public_embed_settings(embed_token.settings)
