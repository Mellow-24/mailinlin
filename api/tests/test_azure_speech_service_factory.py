...
def test_create_azure_speech_services_for_hong_kong_cantonese():
    user_config = SimpleNamespace(
        tts=SimpleNamespace(
            provider=ServiceProviders.AZURE_SPEECH.value,
            api_key="test-subscription-key",
    ...
    with (
        patch("api.services.pipecat.service_factory.AzureTTSService") as mock_tts,
        patch("api.services.pipecat.service_factory.AzureSTTService") as mock_stt,
    ):
        create_tts_service(user_config, _audio_config())
        create_stt_service(user_config, _audio_config())

    tts_kwargs = mock_tts.call_args.kwargs
    assert tts_kwargs["region"] == "eastasia"
    assert tts_kwargs["settings"].voice == "zh-HK-HiuMaanNeural"
    assert tts_kwargs["settings"].language == "zh-HK"

    stt_kwargs = mock_stt.call_args.kwargs
    assert stt_kwargs["region"] == "eastasia"
    assert stt_kwargs["settings"].language == "zh-HK"
