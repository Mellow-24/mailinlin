...
async def update_user_configurations(
    ...
    if incoming_dict:
        ...
        # Derive the replacement from the exact row revision that the guarded
        # write will later save. A concurrent full-config update therefore
        # returns a retryable conflict instead of being silently overwritten.
        model_snapshot = await get_organization_ai_model_configuration_v2_snapshot(
            user.selected_organization_id
        )
        if model_snapshot.configuration is None:
            existing_config = EffectiveAIModelConfiguration()
        else:
            existing_config = compile_ai_model_configuration_v2(
                model_snapshot.configuration
            )
        ...
        try:
            await upsert_organization_ai_model_configuration_v2(
                ...
                expected_snapshot=model_snapshot,
            )
        except OrganizationConfigurationConflictError:
            raise HTTPException(
                status_code=409,
                detail="Model configuration changed while saving. Refresh and try again.",
            ) from None
    else:
