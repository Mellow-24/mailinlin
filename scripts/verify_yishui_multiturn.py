#!/usr/bin/env python3
"""Run a small real-model text regression for the published 易水 workflow."""
...
async def main() -> None:
    ...
    opening_ok = bool(_latest_assistant_text(text_session))

    text_session = await append_text_chat_user_message(
        run_id=run.id,
        text_session=text_session,
        user_text="我想了解卧室的床应该怎样摆放。",
    ...
    first_ok = bool(_latest_assistant_text(text_session))

    text_session = await append_text_chat_user_message(
        run_id=run.id,
        text_session=text_session,
        user_text="我刚才问的是哪个房间？请直接回答。",
        expected_revision=text_session.revision,
    )
    text_session = await execute_pending_text_chat_turn(
        workflow_id=args.workflow_id,
    ...
    second_text = _latest_assistant_text(text_session)
    second_ok = bool(second_text)
    remembers_room = any(term in second_text for term in ("卧室", "臥室", "睡房"))
    ...
    print(
        {
            ...
            "first_reply_ok": first_ok,
            "second_reply_ok": second_ok,
            "second_reply_remembers_room": remembers_room,
