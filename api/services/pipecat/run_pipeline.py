...
from api.services.pipecat.direct_faq_retrieval import (
...
from api.services.workflow.memory import hydrate_memory_context
...
def _should_prewarm_llm(*, fast_opening: bool, is_realtime: bool) -> bool:
    return fast_opening and not is_realtime
...
def _resolve_user_turn_stop_timeout(
    ...
    if "user_turn_stop_timeout" in run_configs:
        configured = float(run_configs["user_turn_stop_timeout"])
        # External STT sends system start/stop frames around a normal-priority
        # transcription frame. A very short watchdog can close an empty turn
        # before that final text drains through the audio backlog. This lower
        # bound does not slow healthy turns: final text still commits at once.
        return max(configured, 5.0) if uses_external_turns else configured
...
async def _run_pipeline_impl(
    ...
    merged_call_context_vars = await hydrate_memory_context(
        db_client=db_client,
        workflow_configurations=run_configs,
        organization_id=workflow.organization_id,
        workflow_id=workflow_id,
    ...
    fast_opening = bool(
        not is_realtime
        and merged_call_context_vars.get(FAST_OPENING_CONTEXT_KEY) is True
    )
    client_opening = bool(
        fast_opening
        and merged_call_context_vars.get(CLIENT_OPENING_CONTEXT_KEY) is True
    )
    direct_voice_demo = bool(
        fast_opening
        and merged_call_context_vars.get(DIRECT_VOICE_DEMO_CONTEXT_KEY) is True
    )
    if fast_opening:
        enable_deferred_stt = getattr(stt, "enable_deferred_start", None)
        # The direct demo plays its opening locally, so use that time to finish
        # STT startup before the microphone becomes available. Deferred STT
        # made the first recorded turn wait behind a late provider handshake.
        if callable(enable_deferred_stt) and not direct_voice_demo:
            enable_deferred_stt()
        enable_tts_priming = getattr(tts, "enable_task_priming", None)
        if callable(enable_tts_priming):
            enable_tts_priming()
    ...
    direct_consultation_questions = (
        3 if run_configs.get("direct_consultation_question_limit") == 3 else 0
    )
    configured_faq_document_uuids = run_configs.get(
        "direct_knowledge_document_uuids", []
    )
    if not isinstance(configured_faq_document_uuids, list):
        configured_faq_document_uuids = []
    direct_faq_document_uuids = [
        document_uuid.strip()
        for document_uuid in configured_faq_document_uuids
        if isinstance(document_uuid, str) and document_uuid.strip()
    ]
    direct_local_faq_enabled = (
        run_configs.get("direct_local_faq_enabled") is True
    )
    ...
    async def _configure_direct_llm_turn(user_turn: int) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Could not apply direct consultation turn token limit "
                f"{turn_limit}: {exc}"
            )
    ...
    async def _prepare_direct_turn_context(_user_turn: int, user_text: str) -> None:
        ...
        if not direct_user_processor.current_turn_is_consultation:
            reference = ""
        else:
            # Search the latest answer rather than the whole transcript.
            # Treating every prior answer equally made an early zodiac hit
            # "stick" across later corrections and polite closing turns.
            reference = await prepare_direct_faq_reference(
                query=user_text,
                organization_id=workflow.organization_id,
                document_uuids=direct_faq_document_uuids,
                embeddings_api_key=embeddings_api_key,
                embeddings_model=embeddings_model,
                embeddings_base_url=embeddings_base_url,
                embeddings_provider=embeddings_provider,
    ...
    direct_response_guard = (
        ConsultationResponseGuardProcessor(
            turn_provider=lambda: direct_user_processor.consultation_turn_count,
            user_text_provider=lambda: direct_user_processor.last_user_text,
            consultation_text_provider=lambda: direct_user_processor.consultation_text,
            faq_reference_provider=_current_direct_faq_reference,
            consultation_response_provider=(
                lambda: direct_user_processor.current_turn_is_consultation
            ),
        )
        if direct_user_processor is not None and direct_consultation_questions == 3
        else None
    )
    ...
    if is_realtime:
        ...
        if fast_opening and uses_external_turns:
            # DashScope owns turn boundaries for this PTT surface; a second
            # local VAD only adds raw-audio work ahead of final transcription.
        ...
        user_turn_start_strategies = _create_non_realtime_user_turn_start_strategies(
            ...
            # Headless push-to-talk cannot begin while the bot is speaking.
            # Disabling a redundant interruption prevents a final-only ASR
            # frame from being cleared between adjacent external start/stop
            # system frames before the user aggregator can commit it.
            enable_interruptions=not fast_opening,
        )
        turn_start_strategy = run_configs.get(
            "turn_start_strategy", DEFAULT_TURN_START_STRATEGY
        )
    ...
    if is_realtime:
        ...
        pipeline = build_pipeline(
            ...
            direct_user_processor=direct_user_processor,
            direct_response_guard=direct_response_guard,
        )
    ...
    if fast_opening and stt is not None:

        async def _send_transcriber_ready(ready: bool) -> None:
            if ws_sender:
                try:
                    await ws_sender(
                        {
                            "type": "rtf-transcriber-ready",
                            "payload": {"ready": ready},
                        }
                    )
                except Exception as exc:
                    logger.debug(
                        f"Failed to send transcriber readiness for run "
                        f"{workflow_run_id}: {exc}"
                    )

        @stt.event_handler("on_connected")
        async def _on_stt_connected(_stt) -> None:
            await _send_transcriber_ready(True)

        @stt.event_handler("on_disconnected")
        async def _on_stt_disconnected(_stt) -> None:
            await _send_transcriber_ready(False)

        @stt.event_handler("on_connection_error")
        async def _on_stt_connection_error(_stt, _error: str) -> None:
            await _send_transcriber_ready(False)

    # Pipecat starts processors in pipeline order. DashScope STT can take a few
    # seconds to acknowledge its task, which previously meant the downstream
    # DashScope TTS WebSocket did not even begin connecting until STT finished.
    # Start an idempotent TTS socket handshake alongside the runner so the fixed
    # opening greeting is ready as soon as StartFrame reaches TTS. Fast-opening
    # push-to-talk runs also acknowledge an empty provider task up front, which
    # the first utterance adopts without another run-task round trip.
    tts_prewarm_task = None
    tts_warmup_method = "prime" if fast_opening else "prewarm"
    tts_prewarm = (
        getattr(tts, tts_warmup_method, None) if not is_realtime else None
    )
    if callable(tts_prewarm):
        ...
        async def _prewarm_tts() -> None:
            try:
                ...
                # Normal start() remains the retry path; prewarming is only a
                # latency optimization and must never prevent a call.
                logger.warning(
                    f"TTS prewarm failed for run {workflow_run_id}; "
                    f"pipeline start will retry: {exc}"
                )

        tts_prewarm_task = asyncio.create_task(
            _prewarm_tts(), name=f"tts-prewarm-{workflow_run_id}"
        )

    llm_prewarm_task = None
    llm_prewarm = (
        getattr(llm, "prewarm", None)
        if _should_prewarm_llm(
    ...
    if callable(llm_prewarm):
        ...
        async def _prewarm_llm() -> None:
            try:
                ...
                logger.warning(
                    f"LLM prewarm failed for run {workflow_run_id}; "
                    f"first user turn will continue normally: {exc}"
                )
    ...
    try:
        ...
        if llm_prewarm_task is not None:
            if not llm_prewarm_task.done():
                llm_prewarm_task.cancel()
            await asyncio.gather(llm_prewarm_task, return_exceptions=True)
