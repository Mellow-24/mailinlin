...
class TestLLMPrewarmPolicy(TestCase):
    def test_fast_opening_non_realtime_prewarms_llm(self):
        policy = getattr(run_pipeline_module, "_should_prewarm_llm", None)

        self.assertIsNotNone(policy)
        self.assertTrue(policy(fast_opening=True, is_realtime=False))

    def test_other_modes_do_not_prewarm_llm(self):
        policy = getattr(run_pipeline_module, "_should_prewarm_llm", None)

        self.assertIsNotNone(policy)
        self.assertFalse(policy(fast_opening=False, is_realtime=False))
        self.assertFalse(policy(fast_opening=True, is_realtime=True))
        self.assertFalse(policy(fast_opening=False, is_realtime=True))
...
def test_non_realtime_push_to_talk_preserves_final_only_external_transcript():
    strategies = _create_non_realtime_user_turn_start_strategies(
        {},
        uses_external_turns=True,
    ...
    assert len(strategies) == 1
    assert isinstance(strategies[0], ExternalUserTurnStartStrategy)
    assert strategies[0]._enable_interruptions is False
...
def test_external_turn_timeout_rejects_unsafe_subsecond_override():
    assert (
        _resolve_user_turn_stop_timeout(
