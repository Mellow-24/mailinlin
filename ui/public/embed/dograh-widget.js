...
(function() {
  ...
  const state = {
    ...
    // Voice conversation state (widgetType === 'voice'). Media stays connected
    // for the whole call, while isRecording only controls whether the local
    // microphone track is currently sent to the agent.
    voice: {
      messages: [],
      isRecording: false,
      agentReady: false,
  ...
  async function initializeWidget() {
    ...
    try {
      ...
      state.config = {
        ...
        fastOpening: Boolean(configData.settings?.fastOpening),
        clientOpeningAssetId: configData.settings?.clientOpeningAssetId || null,
        clientOpeningAudioUrl: configData.settings?.clientOpeningAudioUrl || null,
        clientOpeningTranscript: configData.settings?.clientOpeningTranscript || null,
        clientOpeningDurationMs: Number(configData.settings?.clientOpeningDurationMs) || null,
        recordingDrainMs: Math.max(
          0,
          Math.min(3000, Number(configData.settings?.recordingDrainMs) || 0)
        ),
  ...
  function microphoneTracks() {
    ...
    if (typeof state.stream.getAudioTracks === 'function') {
      return state.stream.getAudioTracks();
    }
    return state.stream.getTracks().filter(track => !track.kind || track.kind === 'audio');
  }

  function voiceMessagesSnapshot() {
    return state.voice.messages.map(message => ({ ...message }));
  }

  function notifyVoiceMessage(message) {
    if (state.callbacks.onVoiceMessage) {
      state.callbacks.onVoiceMessage({ ...message }, voiceMessagesSnapshot());
    }
  }

  function setRecordingState(isRecording) {
    if (state.voice.isRecording === isRecording) return;
    state.voice.isRecording = isRecording;
    if (state.callbacks.onRecordingChange) {
      state.callbacks.onRecordingChange(isRecording);
    }
  }

  function setAgentReady(agentReady) {
    if (state.voice.agentReady === agentReady) return;
    state.voice.agentReady = agentReady;
    if (state.callbacks.onAgentReady) {
      state.callbacks.onAgentReady(agentReady);
    }
  }

  function refreshAgentReady() {
    setAgentReady(Boolean(
      state.connectionStatus === 'connected' &&
      state.voice.openingComplete &&
      state.voice.transcriberReady &&
      !state.voice.botSpeaking
    ));
  }
  ...
  function completeClientOpening(attemptId) {
    if (
      !state.config.clientOpeningAudioUrl ||
      !state.voice.openingAttemptActive ||
      attemptId !== state.voice.openingAttemptId
    ) {
      return false;
    }
    state.voice.openingAttemptActive = false;
    state.voice.openingComplete = true;
    refreshAgentReady();
    return true;
  }
  ...
  function resetVoiceState() {
    ...
    state.voice.botSpeaking = false;
    state.voice.openingComplete = false;
    state.voice.openingAttemptActive = false;
    state.voice.transcriberReady = !state.config.fastOpening;
    state.voice.openingTranscript = null;
    state.voice.nextMessageId = 1;
    state.voice.activeUserMessageId = null;
  }

  function createVoiceMessage(type, text, final) {
    const message = {
      id: `voice-${type}-${state.voice.nextMessageId++}`,
      type: type,
      role: type,
      text: text,
      final: Boolean(final),
    ...
    state.voice.messages = [...state.voice.messages, message];
    notifyVoiceMessage(message);
    return message;
  }

  function replaceVoiceMessage(index, changes) {
    const message = { ...state.voice.messages[index], ...changes };
    state.voice.messages = [
      ...state.voice.messages.slice(0, index),
      message,
      ...state.voice.messages.slice(index + 1)
    ];
    notifyVoiceMessage(message);
    return message;
  }
  ...
  function finalizePendingAssistantMessage() {
    const index = state.voice.messages.length - 1;
    const last = state.voice.messages[index];
    if (last && last.type === 'assistant' && !last.final) {
      replaceVoiceMessage(index, { final: true });
    }
  }

  function activeUserMessageIndex() {
    if (!state.voice.activeUserMessageId) return -1;
    return state.voice.messages.findIndex(
      message => message.id === state.voice.activeUserMessageId
    );
  }

  function reserveUserMessage() {
    const previousIndex = activeUserMessageIndex();
    if (previousIndex >= 0) {
      replaceVoiceMessage(previousIndex, { final: true });
    }
    const message = createVoiceMessage('user', '正在聆听…', false);
    state.voice.activeUserMessageId = message.id;
  }

  function updateActiveUserMessage(text, final) {
    const index = activeUserMessageIndex();
    if (index < 0) return null;
    const message = replaceVoiceMessage(index, {
      text: text,
      final: Boolean(final)
    });
    if (final) state.voice.activeUserMessageId = null;
    return message;
  }

  function stopLocalSpeechRecognition(abort = false) {
    const recognition = state.voice.localRecognition;
    state.voice.localRecognition = null;
    ...
    try {
      ...
      // The server transcript remains authoritative when the visual helper
  ...
  function startLocalSpeechRecognition() {
    ...
    try {
      ...
      recognition.onresult = event => {
        if (recognition !== state.voice.localRecognition) return;
        let transcript = '';
        for (let index = 0; index < event.results.length; index += 1) {
          transcript = joinVoiceTextFragments(
            transcript,
            String(event.results[index]?.[0]?.transcript || '').trim()
          );
        }
        if (transcript) updateActiveUserMessage(transcript, false);
      };
      recognition.onerror = () => {
        if (recognition === state.voice.localRecognition) {
          state.voice.localRecognition = null;
        }
      };
      recognition.onend = () => {
      ...
      state.voice.localRecognition = recognition;
      recognition.start();
    } catch (_) {
      state.voice.localRecognition = null;
    }
  ...
  function handleUserTranscription(payload = {}) {
    ...
    // Update the fixed slot created when recording started. This keeps the
    // customer sentence before the assistant even when the server's final
    // transcript arrives after the first response text.
    if (updateActiveUserMessage(text, Boolean(payload.final))) {
      return;
    }

    // Some ASR providers repeat the final result. Do not turn that retry into
    // a duplicate customer message.
    const duplicate = state.voice.messages.find(
      message => message.type === 'user' && message.final && message.text === text
    );
    if (payload.final && duplicate) {
      return;
    }
    const message = createVoiceMessage('user', text, payload.final);
    if (!payload.final) state.voice.activeUserMessageId = message.id;
  }

  function joinVoiceTextFragments(currentText, fragment) {
    if (!currentText) return fragment;
    if (!fragment) return currentText;
    if (/\s$/.test(currentText) || /^\s/.test(fragment)) return currentText + fragment;

    // Chinese/Japanese text and punctuation should not gain artificial spaces;
    // Latin-language sentence chunks remain readable when providers omit one.
    const cjk = /[\u3000-\u30ff\u3400-\u9fff\uf900-\ufaff\uff00-\uffef]/;
    const punctuation = /^[,.;:!?，。！？、；：)\]}]/;
    if (cjk.test(currentText.slice(-1)) || cjk.test(fragment.charAt(0)) || punctuation.test(fragment)) {
      return currentText + fragment;
    }
    return `${currentText} ${fragment}`;
  }

  function handleBotText(payload = {}) {
    const text = typeof payload.text === 'string' ? payload.text.trim() : '';
    if (!text) return;

    const lastIndex = state.voice.messages.length - 1;
    const last = state.voice.messages[lastIndex];
    if (last && last.type === 'assistant' && !last.final) {
      replaceVoiceMessage(lastIndex, {
        text: joinVoiceTextFragments(last.text, text)
      });
      return;
    }
    createVoiceMessage('assistant', text, false);
  }

  /**
   * Enable the already-negotiated microphone track for one customer turn.
   */
  function startRecording() {
    if (
      isChatWidget() ||
      state.connectionStatus !== 'connected' ||
      !state.voice.agentReady ||
      state.voice.botSpeaking
    ) {
      return false;
    }
    const tracks = microphoneTracks();
  ...
  /**
   * Mute the local microphone without tearing down WebRTC or conversation
   * context. Returns whether a recording turn was active before this call.
   */
  function stopRecording() {
    ...
    if (
      wasRecording &&
      state.config.embedMode === 'headless' &&
      drainMs > 0
    ) {
      state.voice.isDraining = true;
      if (state.voice.drainTimer !== null) {
        clearTimeout(state.voice.drainTimer);
      }
      // Keep the sender alive briefly so the ASR receives more than its
      // configured 800ms trailing-silence boundary and can emit final text.
      state.voice.drainTimer = setTimeout(finishMicrophoneDrain, drainMs);
    } else {
      finishMicrophoneDrain();
    }
    return wasRecording;
  ...
  async function startCall() {
    resetVoiceState();
    state.voice.openingAttemptId += 1;
    state.voice.openingAttemptActive = Boolean(state.config.clientOpeningAudioUrl);
    ...
    try {
      ...
      try {
        ...
        // Headless voice embeds use push-to-talk. Keep their WebRTC sender
        // alive but muted until the host UI explicitly starts the customer's
        // first turn. Existing floating/inline voice widgets retain their
        // continuous-microphone behaviour.
        if (state.config.embedMode === 'headless') {
  ...
  async function initializeEmbedSession() {
    ...
    state.voice.openingTranscript =
      typeof data.opening_transcript === 'string' && data.opening_transcript.trim()
        ? data.opening_transcript.trim()
        : null;
    if (state.voice.openingTranscript && state.callbacks.onOpeningTranscript) {
      state.callbacks.onOpeningTranscript(state.voice.openingTranscript);
    }
  ...
  async function handleWebSocketMessage(message) {
    switch (message.type) {
      ...
      case 'rtf-user-transcription':
        handleUserTranscription(message.payload);
        break;

      case 'rtf-bot-text':
        handleBotText(message.payload);
        break;

      // The *-speaking names are emitted by the current backend. Keep the
      // shorter aliases for backwards-compatible/self-hosted deployments.
      case 'rtf-bot-started':
      case 'rtf-bot-started-speaking':
        state.voice.botSpeaking = true;
        refreshAgentReady();
        if (state.config.embedMode === 'headless') {
          finishMicrophoneDrain();
          setRecordingState(false);
      ...
      case 'rtf-transcriber-ready':
        state.voice.transcriberReady = message.payload?.ready !== false;
        refreshAgentReady();
        break;
  ...
  window.DograhWidget = {
    ...
    // Headless voice API. A call keeps WebRTC connected while these methods
    // toggle only the microphone sender for push-to-talk customer turns.
    getVoiceMessages: () => voiceMessagesSnapshot(),
    getOpeningTranscript: () => state.voice.openingTranscript,
    getClientOpening: () => (
      state.config.clientOpeningAudioUrl && state.config.clientOpeningTranscript
        ? {
            assetId: state.config.clientOpeningAssetId,
            audioUrl: state.config.clientOpeningAudioUrl,
            transcript: state.config.clientOpeningTranscript,
            durationMs: state.config.clientOpeningDurationMs
          }
        : null
    ),
    getOpeningAttemptId: () => state.voice.openingAttemptId,
    completeClientOpening: completeClientOpening,
    onVoiceMessage: (callback) => { state.callbacks.onVoiceMessage = callback; },
    onOpeningTranscript: (callback) => {
      state.callbacks.onOpeningTranscript = callback;
      if (callback && state.voice.openingTranscript) {
        callback(state.voice.openingTranscript);
      }
    },
    onAgentReady: (callback) => {
      state.callbacks.onAgentReady = callback;
      if (callback && state.voice.agentReady) callback(true);
    },
    onRecordingChange: (callback) => {
      state.callbacks.onRecordingChange = callback;
      if (callback && state.voice.isRecording) callback(true);
    },
