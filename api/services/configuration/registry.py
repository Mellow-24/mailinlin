...
class BaseServiceConfiguration(BaseModel):
    provider: Literal[
        ...
        ServiceProviders.DASHSCOPE,
        ServiceProviders.MINISTREAM,
    ]
...
DASHSCOPE_PROVIDER_MODEL_CONFIG = provider_model_config(
    "Alibaba Cloud Model Studio (DashScope)",
    description=(
        "Alibaba Cloud Model Studio models through DashScope. A regional "
        "DashScope API key is required."
    ),
    provider_docs_url="https://help.aliyun.com/zh/model-studio/",
)
MINISTREAM_PROVIDER_MODEL_CONFIG = provider_model_config(
    "MiniStream",
    description=(
        "MiniStream trial WebSocket text-to-speech with a Cantonese preset voice. "
        "The fixed trial endpoint uses a pinned self-signed TLS certificate."
    ),
)
...
@register_llm
class DashScopeLLMConfiguration(BaseLLMConfiguration):
    model_config = DASHSCOPE_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.DASHSCOPE] = ServiceProviders.DASHSCOPE
    model: str = Field(
        default=DASHSCOPE_LLM_MODELS[0],
        description="Qwen chat model deployed in Alibaba Cloud Model Studio.",
        json_schema_extra={
            "examples": DASHSCOPE_LLM_MODELS,
    ...
    base_url: str = Field(
        ...
        description=(
            "Regional DashScope OpenAI-compatible base URL. The API key and URL "
    ...
    temperature: float = Field(
        ...
        ge=0.0,
        le=2.0,
        description="Sampling temperature for Qwen responses.",
    )
    enable_thinking: bool = Field(
        default=False,
        description=(
            "Enable Qwen reasoning before answering. Keep disabled for low-latency "
            "voice conversations."
        ),
    )
    max_tokens: int = Field(
        ...
        ge=16,
        le=4096,
        description="Maximum response tokens; 256 is suitable for short voice replies.",
    )
...
@register_tts
class MiniStreamTTSConfiguration(BaseTTSConfiguration):
    model_config = MINISTREAM_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.MINISTREAM] = ServiceProviders.MINISTREAM
    model: str = Field(
        default=MINISTREAM_TTS_MODELS[0],
        description="Configuration placeholder for MiniStream TTS.",
        json_schema_extra={"examples": MINISTREAM_TTS_MODELS},
    )
    language: Literal["yue"] = Field(
        default=MINISTREAM_TTS_LANGUAGES[0],
        description="MiniStream language code for Cantonese synthesis.",
        json_schema_extra={"examples": MINISTREAM_TTS_LANGUAGES},
    )
    generation_mode: Literal["preset_voice"] = Field(
        default=MINISTREAM_GENERATION_MODES[0],
        description="Use MiniStream's preset-voice generation mode.",
        json_schema_extra={"examples": MINISTREAM_GENERATION_MODES},
    )
    voice_preset_key: str = Field(
        default=MINISTREAM_VOICE_PRESET_KEYS[0],
        description="MiniStream preset voice key.",
        json_schema_extra={"examples": MINISTREAM_VOICE_PRESET_KEYS},
    )
    buffer: Literal["off"] = Field(
        default=MINISTREAM_BUFFER_MODES[0],
        description="Disable provider-side text buffering for streaming TTS.",
        json_schema_extra={"examples": MINISTREAM_BUFFER_MODES},
    )
    buffer_idle_ms: int = Field(
        default=250,
        ge=0,
        description="Flush buffered text after this idle period in milliseconds.",
    )
    max_generate_length: int = Field(
        default=500,
        ge=1,
        description="Maximum text length sent in each MiniStream generation request.",
    )
    websocket_url: str = Field(
        default=MINISTREAM_WEBSOCKET_URL,
        description=(
            "MiniStream WebSocket base path. The service appends the client ID; "
            "do not add one here."
        ),
        json_schema_extra={"allow_custom_input": True},
    )
    http_base_url: str = Field(
        default=MINISTREAM_HTTP_BASE_URL,
        description="MiniStream HTTP API base URL used only for opening audio generation.",
        json_schema_extra={"allow_custom_input": True},
    )
    sample_rate: Literal[16000] = Field(
        default=16000,
        description="Target PCM sample rate for the voice pipeline.",
    )
    playback_rate: float = Field(
        default=1.0,
        gt=0,
        description="Local playback speed multiplier for MiniStream audio.",
    )
    verify_ssl: bool = Field(
        default=True,
        description=(
            "Require verified TLS for MiniStream connections. The fixed trial "
            "endpoint uses its configured certificate fingerprint; custom "
            "endpoints use normal CA and hostname verification."
        ),
    )
    tls_certificate_fingerprint_sha256: str = Field(
        default=MINISTREAM_TRIAL_TLS_FINGERPRINT_SHA256,
        description=(
            "SHA-256 certificate fingerprint used only for the fixed MiniStream "
            "trial endpoint."
        ),
        json_schema_extra={"allow_custom_input": False},
    )

    @field_validator("tls_certificate_fingerprint_sha256")
    @classmethod
    def validate_trial_tls_fingerprint(cls, value: str) -> str:
        normalized_value = value.replace(":", "").upper()
        if normalized_value != MINISTREAM_TRIAL_TLS_FINGERPRINT_SHA256:
            raise ValueError(
                "MiniStream trial TLS certificate fingerprint must match the "
                "configured trial certificate"
            )
        return normalized_value

    @model_validator(mode="after")
    def require_secure_tls_endpoint_configuration(self):
        if not self.verify_ssl:
            raise ValueError("verify_ssl=True is required for MiniStream TTS")

        for url, required_scheme, label in (
            (self.websocket_url, "wss", "websocket_url"),
            (self.http_base_url, "https", "http_base_url"),
        ):
            parsed_url = urlsplit(url)
            if parsed_url.scheme != required_scheme:
                raise ValueError(
                    f"MiniStream {label} must use "
                    f"{'WSS' if required_scheme == 'wss' else 'HTTPS'}"
                )
            if (
                ...
                raise ValueError(
                    f"MiniStream {label} must be a credential-free base URL"
                )
        return self


@register_tts
class DashScopeTTSConfiguration(BaseTTSConfiguration):
    model_config = DASHSCOPE_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.DASHSCOPE] = ServiceProviders.DASHSCOPE
    model: str = Field(
        default=DASHSCOPE_TTS_MODELS[0],
        description="CosyVoice realtime speech-synthesis model.",
        json_schema_extra={
            "examples": DASHSCOPE_TTS_MODELS,
            "allow_custom_input": True,
        },
    )
    voice: str = Field(
        default="longjiaxin_v3",
        description="CosyVoice speaker name. The default is a Beijing-region Cantonese female voice.",
        json_schema_extra={
            "examples": DASHSCOPE_CANTONESE_VOICES,
            "allow_custom_input": True,
        },
    )
    websocket_url: str = Field(
        default=DASHSCOPE_WEBSOCKET_URL,
        description=(
            "Regional DashScope WebSocket inference URL. The API key and URL "
            "must belong to the same region."
        ),
        json_schema_extra={"allow_custom_input": True},
    )
    sample_rate: Literal[24000] = Field(
        default=24000,
        description="CosyVoice raw PCM output sample rate used by the voice pipeline.",
    )
    audio_format: Literal["pcm"] = Field(
        default="pcm",
        description="Raw signed 16-bit PCM required by the realtime audio pipeline.",
    )
    instruction: str | None = Field(
        default=None,
        description=(
            "Optional Qwen-Audio-TTS/CosyVoice instruction. Leave blank for longjiaxin_v3, "
            "longjiayi_v3, and longanyue_v3 because these system voices do not "
            "support instruction control."
        ),
    )
    rate: float = Field(
        default=1.0,
        ge=0.5,
        le=2.0,
        description="Speech speed multiplier for DashScope streaming TTS.",
    )
    workspace_id: str | None = Field(
        default=None,
        description="Optional Alibaba Cloud Model Studio workspace ID.",
    )
...
@register_stt
class DashScopeSTTConfiguration(BaseSTTConfiguration):
    model_config = DASHSCOPE_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.DASHSCOPE] = ServiceProviders.DASHSCOPE
    model: str = Field(
        default=DASHSCOPE_STT_MODELS[0],
        description="DashScope realtime speech-recognition model.",
        json_schema_extra={
            "examples": DASHSCOPE_STT_MODELS,
            "allow_custom_input": True,
        },
    )
    language: str = Field(
        default="zh",
        description=(
            "Recognition language hint. Fun-ASR uses zh for Chinese, including "
            "Cantonese; the 2026-02-28 snapshot does not accept yue as a hint."
        ),
        json_schema_extra={
            "examples": ["zh", "en", "ja"],
            "allow_custom_input": True,
    ...
    sample_rate: Literal[16000] = Field(
        default=16000,
        description="16 kHz microphone PCM input expected by the ASR model.",
    )
    audio_format: Literal["pcm"] = Field(
        default="pcm",
        description="Raw signed 16-bit mono PCM input.",
    )
    vocabulary_id: str | None = Field(
        default=None,
        description="Optional DashScope hot-word vocabulary ID.",
    )
    workspace_id: str | None = Field(
        default=None,
        description="Optional Alibaba Cloud Model Studio workspace ID.",
...
@register_embeddings
class DashScopeEmbeddingsConfiguration(BaseEmbeddingsConfiguration):
    model_config = DASHSCOPE_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.DASHSCOPE] = ServiceProviders.DASHSCOPE
    model: str = Field(
        default=DASHSCOPE_EMBEDDING_MODELS[0],
        description=(
            "DashScope text embedding model. Dograh explicitly requests 1536 "
            "dimensions to match its vector database schema."
        ),
        json_schema_extra={
            "examples": DASHSCOPE_EMBEDDING_MODELS,
            "allow_custom_input": True,
        },
    )
    base_url: str = Field(
    ...
    dimensions: Literal[DASHSCOPE_EMBEDDING_DIMENSION] = Field(
        default=DASHSCOPE_EMBEDDING_DIMENSION,
        description="Fixed to 1536 to match Dograh's pgvector column.",
    )
