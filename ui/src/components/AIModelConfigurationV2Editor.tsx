...
import { Info, KeyRound, Languages, Save } from "lucide-react";
...
function buildHongKongCantonesePipelinePreset(
    config: Record<string, unknown> | null,
): Record<string, unknown> {
    const currentTts = asRecord(config?.tts);
    const currentStt = asRecord(config?.stt);
    const azureTts = currentTts?.provider === "azure_speech" ? currentTts : null;
    const azureStt = currentStt?.provider === "azure_speech" ? currentStt : null;
    ...
    return {
        ...
        tts: {
            ...
            model: "neural",
            region: String(azureTts?.region || "eastasia"),
        ...
        stt: {
            ...
            provider: "azure_speech",
            model: "latest_long",
            region: String(azureStt?.region || "eastasia"),
            language: "zh-HK",
        },
    };
}

function buildAlibabaFullStackPipelinePreset(
    config: Record<string, unknown> | null,
): Record<string, unknown> {
    const preserveDashScopeKey = (service: "llm" | "tts" | "stt" | "embeddings") => {
        const current = asRecord(config?.[service]);
        return current?.provider === "dashscope" ? current : null;
    };
    const llm = preserveDashScopeKey("llm");
    const tts = preserveDashScopeKey("tts");
    const stt = preserveDashScopeKey("stt");
    const embeddings = preserveDashScopeKey("embeddings");
    ...
    return {
        ...
        llm: {
            ...
            base_url: String(llm?.base_url || compatibleBaseUrl),
            temperature: 0.1,
        },
        tts: {
            ...(tts ?? {}),
            ...
            websocket_url: String(tts?.websocket_url || websocketUrl),
            voice: "longjiaxin_v3",
            sample_rate: 24000,
            audio_format: "pcm",
            // This Cantonese system voice does not support CosyVoice instruction control.
            instruction: "",
        },
        stt: {
            ...(stt ?? {}),
            ...
            websocket_url: String(stt?.websocket_url || websocketUrl),
            language: "zh",
            sample_rate: 16000,
            audio_format: "pcm",
        },
        embeddings: {
            ...
            base_url: String(embeddings?.base_url || compatibleBaseUrl),
            dimensions: 1536,
        },
    };
}
        ...
        <div className="space-y-6">
            ...
            <Tabs value={mode} onValueChange={(value) => setMode(value as ModelMode)} className="space-y-6">
                ...
                <TabsContent value="byok" className="mt-0">
                    ...
                    <PricingSummary pricing={pricing} includeDograhModel={false} thirdPartyModels />
                    <div className="mb-4 flex flex-col gap-3 rounded-xl border border-sky-500/30 bg-sky-500/[0.06] p-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <p className="font-medium">阿里百炼全栈预设</p>
                            <p className="mt-1 text-sm text-muted-foreground">
                                预填千问、Fun-ASR、CosyVoice 与 1536 维向量模型。四项服务使用同一枚百炼 API Key。
                            </p>
                        </div>
                        <Button
                            type="button"
                            variant="outline"
                            className="shrink-0"
                            onClick={() => setPipelineInitialConfig(
                                buildAlibabaFullStackPipelinePreset(pipelineInitialConfig),
                            )}
                        >
                    ...
                    <div className="mb-4 flex flex-col gap-3 rounded-xl border border-amber-500/30 bg-amber-500/[0.06] p-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <p className="font-medium">香港粤语快捷预设</p>
                            <p className="mt-1 text-sm text-muted-foreground">
                                预填 Azure Speech 的 zh-HK 识别与粤语女声。区域必须与您的 Azure Speech 资源一致。
                            </p>
                        </div>
                        <Button
                            type="button"
                            ...
                            className="shrink-0"
                            onClick={() => setPipelineInitialConfig(
                                buildHongKongCantonesePipelinePreset(pipelineInitialConfig),
                            )}
                        >
                            <Languages className="mr-2 h-4 w-4" />
                            应用粤语预设
