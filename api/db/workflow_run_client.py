...
from api.services.workflow.memory import extract_consented_memory_summary
...
class WorkflowRunClient(BaseDBClient):
    ...
    async def get_latest_consented_memory_summary(
        ...
        *,
        ...
        """Find the newest earlier summary that is safe to reuse.

        The organization/workflow predicates are mandatory tenant boundaries.
        ``memory_subject_id`` is the scoped hash injected at run start; raw phone
        numbers are never used for this lookup.
        """

        async with self.async_session() as session:
            current_result = await session.execute(
                select(WorkflowRunModel.created_at, WorkflowRunModel.id)
                .join(
                    WorkflowModel,
                    WorkflowRunModel.workflow_id == WorkflowModel.id,
                )
                .where(
                    WorkflowRunModel.id == before_run_id,
            ...
            current_created_at, current_id = current_row
            subject_text = WorkflowRunModel.initial_context.op("->>")(
                "memory_subject_id"
            )
            explicit_consent_text = WorkflowRunModel.gathered_context.op("->>")(
                "explicit_memory_consent"
            )
            memory_consent_text = WorkflowRunModel.gathered_context.op("->>")(
                "memory_consent"
            )
            memory_summary_text = WorkflowRunModel.gathered_context.op("->>")(
                "memory_summary"
            )
            session_summary_text = WorkflowRunModel.gathered_context.op("->>")(
                "session_summary"
            )
            ...
            candidates_result = await session.execute(
                ...
                .where(
                    ...
                    subject_text == memory_subject_id,
                    or_(
                        explicit_consent_text == "true",
                        memory_consent_text == "true",
                    ),
                    or_(
                        func.length(func.btrim(memory_summary_text)) > 0,
                        func.length(func.btrim(session_summary_text)) > 0,
                    ),
                    or_(
                        WorkflowRunModel.created_at < current_created_at,
                        and_(
                            WorkflowRunModel.created_at == current_created_at,
                            WorkflowRunModel.id < current_id,
                        ),
                    ),
                )
                .order_by(
            ...
            for candidate in candidates_result.scalars():
                # Revalidate in Python and fail closed if historical JSON has a
                # surprising type despite matching PostgreSQL's text projection.
                summary = extract_consented_memory_summary(candidate.gathered_context)
