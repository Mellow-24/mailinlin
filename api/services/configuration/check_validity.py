...
class UserConfigurationValidator:
    def __init__(self):
        ...
        self._validator_map = {
            ...
            ServiceProviders.DASHSCOPE.value: self._check_dashscope_api_key,
            ServiceProviders.MINISTREAM.value: self._check_ministream_api_key,
        }
    ...
    def _validate_service(
        ...
        for url_field in ("base_url", "endpoint", "websocket_url", "http_base_url"):
    ...
    def _check_dashscope_api_key(self, model: str, api_key: str) -> bool:
        """Validate presence without calling DashScope's inference endpoints.

        DashScope's OpenAI-compatible API intentionally does not expose the
        OpenAI ``GET /models`` endpoint, while its native speech APIs are
        WebSocket inference endpoints. Saving a configuration therefore cannot
        use the generic OpenAI model-list probe without rejecting valid keys or
        making a billable model call. Authentication is verified on the first
        actual request and surfaced through the normal service error path.
        """
        return bool(api_key and api_key.strip())

    def _check_ministream_api_key(self, model: str, api_key: str) -> bool:
        """Validate key presence without issuing a billable MiniStream request."""
        return bool(api_key and api_key.strip())
