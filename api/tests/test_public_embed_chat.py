...
def test_init_voice_token_keeps_smallwebrtc(_patch_db):
    ...
    assert body["opening_transcript"] == "你好，我係易水 AI 顧問。"
    assert body["chat_session"] is None


def test_init_voice_pins_client_opening_from_token_settings(monkeypatch, _patch_db):
    ...
    monkeypatch.setattr(
        ...
        "settings",
        {
            ...
            "clientOpeningTranscript": transcript,
        },
    )
    ...
    assert initial_context[CLIENT_OPENING_CONTEXT_KEY] is True
    assert initial_context[CLIENT_OPENING_TRANSCRIPT_CONTEXT_KEY] == transcript
    assert resp.json()["opening_transcript"] == transcript


def test_init_voice_redacts_a_browser_ambiguous_client_opening_url(
    ...
    monkeypatch.setattr(
        ...
        "settings",
        {
            ...
            "embedMode": "headless",
            "fastOpening": True,
            "clientOpeningAssetId": "greetings-04",
            "clientOpeningAudioUrl": "/\\evil.example/opening.wav",
            "clientOpeningTranscript": transcript,
            "clientOpeningDurationMs": 4321,
        },
    )
    ...
    initial_context = _patch_db.created_runs[0]["initial_context"]
    assert initial_context[CLIENT_OPENING_CONTEXT_KEY] is False
    assert initial_context[CLIENT_OPENING_TRANSCRIPT_CONTEXT_KEY] == ""
    for setting_name in (
        ...
        assert setting_name not in init_response.json()["config"]

    config_response = client.get(
        "/api/v1/public/embed/config/voice",
        headers={"Origin": ORIGIN},
    )

    assert config_response.status_code == 200
    assert "clientOpeningAudioUrl" not in config_response.json()["settings"]


def test_init_voice_does_not_preview_per_call_greeting_override(_patch_db):
    resp = client.post(
        "/api/v1/public/embed/init",
        headers={"Origin": ORIGIN},
        json={
            "token": "voice",
            "context_variables": {
                "greeting_override": {"type": "text", "text": "動態問候"}
            },
        },
    )
    assert resp.status_code == 200
    assert resp.json()["opening_transcript"] is None
